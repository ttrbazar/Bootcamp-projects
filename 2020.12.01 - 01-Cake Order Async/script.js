...
orderBtn.onclick = async () => {
    ...
    if (orderAmountValue > 0) {
        ...
        try {
            let checkOrderResolve = await checkOrder([cakeType.value, orderAmountValue]);
            let paymentResolve = await payment(checkOrderResolve);
            let stockControlResolve = await stockControl(paymentResolve);
            console.log(stockControlResolve);
        } catch (e) {
            console.log(e);
        }
    };
